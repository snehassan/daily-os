...
import { getShiftedTimeLabel, getBlockMinutes } from "@/lib/time";
...
export default function ScheduleBlockComponent({ block, shiftMinutes, isNow, onEdit }: Props) {
  ...
  return (
    ...
    >
      <div className={`py-3 pr-2.5 sm:pr-3.5 lg:pr-4 text-right font-mono text-[10px] sm:text-[11px] lg:text-xs tracking-[0.04em] whitespace-nowrap leading-[1.4]
        ${isNow ? "text-accent-buffer font-medium" : "text-muted"}`}>
        {timeLabel}
        {isNow && (
      ...
      <div className={`py-[11px] px-3 sm:py-[13px] sm:px-[14px_14px_13px_18px] lg:py-[15px] lg:px-[18px_18px_15px_22px]
